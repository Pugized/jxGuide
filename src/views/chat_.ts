...
export class Chat {
  public static bootstrap(info = {}): void {
    ...
    messages.push({
      ...
      sender: AI_ROLE,
      notForApi: true,
      text: `这里是 ${currentInfo.name || "嘉祥外国语学校"}。${
        currentInfo.detail || ""
      }`,
    });
    ...
    function setSendingState(isSending: boolean) {
      ...
      if (isSending) {
        // 切换为停止图标，隐藏文本
        if (sendIconEl) sendIconEl.innerHTML = stopSvg || "停止";
        if (sendBtn) sendBtn.title = "停止";
      } else {
    ...
    async function sendMessage(prompt: string) {
      ...
      if (context) messagesForApi.push({ role: "system", content: context });
    ...
    (function initButtonIcons() {
      try {
        const initSendIcon =
          sendBtn.dataset.sendIcon ||
          sendBtn.querySelector(".btn-icon")?.innerHTML ||
          "";
        const initClearIcon =
          newChatBtn.dataset.clearIcon ||
          newChatBtn.querySelector(".btn-icon")?.innerHTML ||
          "";
        if (sendBtn !== null) {
          let element = sendBtn.querySelector(".btn-icon");
          if (element !== null) element.innerHTML = initSendIcon;
        }

        if (newChatBtn !== null && newChatBtn.querySelector(".btn-icon")) {
          let element = newChatBtn.querySelector(".btn-icon");
          if (element !== null) element.innerHTML = initClearIcon;
        }
