...
export function _init() {
  ...
  function appendTextToNode(node:Element, text:string) {
    node.textContent = node.textContent + text;
  }

  function createMessageElement(type:"user" | "bot" | "error", sender:any, initialText = "") {
  ...
  async function fetchStream(url:string, options:any, onChunk:(chunk: string) => void, signal:any) {
  ...
  // 内存历史
  const messages:Message[] = []; // { type: 'user'|'bot'|'error', sender, text }

  function addMessageToHistory(msg: Message) {
    messages.push(msg);
  }
  function getHistory() {
    // 返回浅拷贝，避免外部直接修改内部数组
  ...
  function clearHistory() {
    if(messages.length>0){
        console.log('kk');
    messages.length = 1;
    const container:any = document.getElementById("chat-messages");
    container.innerHTML='';
    renderMessageNode(container, messages[0] as Message);}
    // else
    // messages.length=0;
  }

  // 创建并插入消息节点，并不重复记录（记录由调用者控制）
  function renderMessageNode(container: any, msg: Message) {
  ...
  (window as any).initChat = function (info = {}) {
    ...
    if (!container || !input || !sendBtn || !clearBtn) {
      // DOM 可能尚未挂载，稍后再试
    ...
    (function initButtonIcons() {
      try {
        ...
        const initClearIcon = clearBtn.dataset.clearIcon || clearBtn.querySelector('.btn-icon')?.innerHTML || '';
        if (sendBtn.querySelector('.btn-icon')) (sendBtn.querySelector('.btn-icon') as Element).innerHTML = initSendIcon;
        if (clearBtn.querySelector('.btn-icon')) (clearBtn.querySelector('.btn-icon') as Element).innerHTML = initClearIcon;
